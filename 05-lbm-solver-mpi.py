...
class lbm_solver:
    ...
    def mpi_commit_border_transfer(self):
        ...
        # Send left border
        if rx > 0:
            self.mpi_comm.isend(f_l, dest=self.mpi_cart.Get_cart_rank([rx - 1, ry]), tag=10)
            self.mpi_comm.isend(rho_l, dest=self.mpi_cart.Get_cart_rank([rx - 1, ry]), tag=20)
            self.mpi_comm.isend(vel_l, dest=self.mpi_cart.Get_cart_rank([rx - 1, ry]), tag=30)

        # Send right border
        if rx < self.mpi_worker_x - 1:
            self.mpi_comm.isend(f_r, dest=self.mpi_cart.Get_cart_rank([rx + 1, ry]), tag=11)
            self.mpi_comm.isend(rho_r, dest=self.mpi_cart.Get_cart_rank([rx + 1, ry]), tag=21)
            self.mpi_comm.isend(vel_r, dest=self.mpi_cart.Get_cart_rank([rx + 1, ry]), tag=31)
        ...
        return reqs, f_cur_np, vel_np, rho_np

    def mpi_wait_border_transfer(self, reqs, f_cur_np, vel_np, rho_np):
        for i, req in enumerate(reqs):
            if i == 0 and req != None:
                ...
                vel_np[-1, :] = req.wait()
        self.f_cur.from_numpy(f_cur_np)
    ...
    def solve(self):
        ...
        for i in range(self.steps):
            handler = self.mpi_commit_border_transfer()
            self.collide_and_stream_bulk()
            self.mpi_wait_border_transfer(*handler)
            ...
            if i % rate == 0:
                ...
                if self.mpi_rank == 0:
                    print(f"Performance {1.0 / t * rate} steps/s", flush=True)
                    if not gui.running:
                        self.mpi_comm.Abort()
                        break
